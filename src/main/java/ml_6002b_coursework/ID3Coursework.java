...
public class ID3Coursework
        ...
        implements TechnicalInformationHandler, Sourcable, Tuneable {
    ...
    private boolean readFlag(String flag, String[] options){
        return Utils.getOptionPos(flag, options) > -1;
    }

    private int determineBranch(Instance instance){
        int temp = m_Attribute.isNumeric() ?
                instance.value(m_Attribute) < randomSplitValue ? 0 : 1
                : (int) instance.value(m_Attribute);
        return temp;
    }

    public String[][] allMeasures(){
        return new String[][]{{"-s", "g"}, {"-s", "g"}, {"-s", "c"}, {"-s", "cy"}};
    }
    ...
    public void setOptions(String[] options) throws Exception {
        switch (Utils.getOption('s', options)){
            ...
            case "g":
                ...
                break;
            case "c":
                attSplit = new ChiSquaredAttributeSplitMeasure(false);
                break;
            case "cy":
                attSplit = new ChiSquaredAttributeSplitMeasure(true);
                break;
        }
//        if (readFlag("ig", options))
//            attSplit = new IGAttributeSplitMeasure();
//        else if (readFlag("g", options))
//            attSplit = new GiniAttributeSplitMeasure();
//        else if (readFlag("c", options)) {
//            boolean yates = readFlag("y", options);
//            attSplit = new ChiSquaredAttributeSplitMeasure(yates);
//        }
    }
    ...
    private void makeTree(Instances data) throws Exception {
        ...
        if (numInstances == 0) {
            ...
            m_Attribute = null;
            m_ClassValue = random.ints(0, numClasses).findFirst().getAsInt(); //Utils.missingValue();
...
//            splitData = attSplit.splitData(data, m_Attribute);
    ...
    public double classifyInstance(Instance instance)
        ...
        if (m_Attribute == null) {
            ...
            return m_Successors[determineBranch(instance)].
    ...
    public double[] distributionForInstance(Instance instance)
        ...
        if (m_Attribute == null) {
            ...
            int temp = determineBranch(instance);
    ...
    public static void main(String[] args) throws Exception {
        ...
        Instances chinatownTrain = loadData("./test_data/Chinatown_TRAIN.arff");
        Instances chinatownTest = loadData("./test_data/Chinatown_TEST.arff");
        ...
        HashMap<Instances, Instances> datasets = new HashMap<>();
        datasets.put(optdigitsTrain, optdigitsTest);
        datasets.put(chinatownTrain, chinatownTest);
        ID3Coursework id3 = new ID3Coursework();

        datasets.forEach((train, test) -> {
            for (String[] measure : id3.allMeasures()) {
                try {
                    id3.setOptions(measure);
                    id3.buildClassifier(train);
                    System.out.println(
                            "Id3 using measure " + id3.attSplit +
                            " on " + train.relationName() +
...
//    System.out.println("Id3 using measure " + "ig" + " on " + "optdigits" + " Problem has test accuracy = " + accuracy(optdigitsTest, id3));
    ...
    public ParameterSpace getDefaultParameterSearchSpace() {
        ...
        String[] splitMeausres={"ig","g","c","cy"};
        ps.addParameter("s", splitMeausres);
        return ps;
    }
}
